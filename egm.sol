...
contract egogaming is Context, IERC20, Ownable {
    ...
    string private _name = "eGoGaming";
    ...
    address public marketingWallet = 0xe6BdA896CCfDcc912373D712F039963eCE103dC2;
    uint256 private _previousmarketingFee = _marketingFee;
	
	uint256 public _ecosystemFee = 3;
    address public ecosystemWallet = 0x90CEEd4F573589c5c2775B38884E335e0767115f;
	...
	function setEcosystemFeePercent(uint256 ecosystemFee) external onlyOwner() {
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _marketingFee==0 && _burnFee==0 && _ecosystemFee ==0) return;
    ...
    function _transfer(
        ...
        _tokenTransfer(from,to,amount,deadWallet);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, address burnWallet) private{
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
		...
		(tTransferAmount, rTransferAmount) = takeEcosystem(sender, tTransferAmount, rTransferAmount, tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        _takeLiquidity(tLiquidity);
        _reflectFee(rFee, tFee);
	...
	function takeEcosystem(address sender, uint256 tTransferAmount, uint256 rTransferAmount, uint256 tAmount) private
    returns (uint256, uint256){
        if(_ecosystemFee==0) {  return(tTransferAmount, rTransferAmount); }
        uint256 tEcosystem = tAmount.div(100).mul(_ecosystemFee);
        uint256 rEcosystem = tEcosystem.mul(_getRate());
        rTransferAmount = rTransferAmount.sub(rEcosystem);
        tTransferAmount = tTransferAmount.sub(tEcosystem);
        _rOwned[ecosystemWallet] = _rOwned[ecosystemWallet].add(rEcosystem);
        emit Transfer(sender, ecosystemWallet, tEcosystem);
        return(tTransferAmount, rTransferAmount);
    }
